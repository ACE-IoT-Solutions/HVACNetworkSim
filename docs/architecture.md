...
For BACnet/IP devices, the simulator now derives `ip-subnet-mask` and `ip-default-gateway` from the configured CIDR instead of relying on fixed `/16` assumptions. Network-port objects can also advertise an explicit `network-number` when `BACNET_NETWORK_NUMBER` is configured.

### Deployment Modes

The simulator currently supports three BACnet deployment patterns:

1. Simple mode: one BACnet/IP device in a single container, optionally with explicit `BACNET_DEVICE_ID` and `BACNET_NETWORK_NUMBER`.
2. Brick mode: one building container with an external BACnet/IP router port and internal routed VLAN-style BACnet networks for AHUs and central plant.
3. Campus mode: one container per building plus BBMD containers and a campus router, generated by `campus/generate_campus.py` and started through `./hvac-sim --campus`.

Non-FDR scanners or drivers attached to a single building subnet need explicit routes for the other campus subnets. For example, a participant on `10.1.0.0/24` should route `10.2.0.0/24` via `10.1.0.102`; otherwise remote broadcasts can reach building 2, but unicast I-Am replies from `10.2.0.0/24` cannot return to the scanner. The campus router also keeps the former `.254` address as a compatibility alias, so routes via `10.1.0.254` still work.

Campus mode also supports built-in scenario profiles:

- `default`: standard two-building campus example
- `multi-network`: standard campus example with explicit BACnet network numbers `100`, `200`, ...
- `multi-network-collisions`: checked-in collision example with duplicate `bacnet:deviceId` annotations across buildings
- `multi-network-bdt-asymmetry`: standard campus example with an asymmetric `BBMD_BDT_PEERS` configuration on `bbmd2`
- `multi-network-duplicate-router-claim`: standard campus example with `sim1` advertising an extra `ROUTER_CLAIMED_NETWORKS=2100`

Every generated campus service also gets a loopback-only fault-control API:

- BBMDs publish `127.0.0.1:1910N`
- building simulators publish `127.0.0.1:1920N`
- `GET /fault/status`
- `POST /fault/silence`
- `POST /fault/resume`

Under the hood this is implemented by a small supervisor process that stays alive as PID 1 and uses `SIGSTOP` / `SIGCONT` on the actual BACnet process. That lets tests force an infrastructure node DOWN and back to OK without tearing the container down or changing the compose topology.

For downstream direct multi-homed tests, the repo also ships `docker-compose.multihomed.yml`, which removes BBMDs and the campus router entirely and leaves network bridging to the attached test client.

### Brick-Driven BACnet Metadata

The Brick parser can now extract explicit BACnet device instance annotations from TTL files:

```turtle
@prefix bacnet: <http://data.ashrae.org/bacnet/2020#> .

campus:Building2_VAV201 a brick:VAV ;
    bacnet:deviceId 100 .
```

When present on AHUs, VAVs, chillers, or boilers, these annotations override the simulator's auto-assigned device-id sequence for those equipment instances. This is primarily used by the checked-in collision scenario.
