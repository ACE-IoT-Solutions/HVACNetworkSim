...
async def simulate_chilled_water_plant(chiller, cooling_tower, app_chiller, app_tower, 
                                      weather_data, ahus, hours_per_minute=60):
...
async def simulate_hot_water_plant(boiler, app_boiler, weather_data, vav_boxes, hours_per_minute=60):
    ...
    try:
        ...
        for vav, app in vav_devices:
            simulation_tasks.append(
                asyncio.create_task(
                    simulate_vav_box(vav, app, weather_data, hours_per_minute=1)
                )
            )
        ...
        simulation_tasks.append(
            asyncio.create_task(
                simulate_ahu(ahu1, ahu_apps[0] if ahu_apps else None, weather_data, 
                             ahu1.vav_boxes, hours_per_minute=1)
            )
        )
        ...
        simulation_tasks.append(
            asyncio.create_task(
                simulate_ahu(ahu2, ahu_apps[1] if len(ahu_apps) > 1 else None, 
                             weather_data, ahu2.vav_boxes, hours_per_minute=1)
            )
        )
