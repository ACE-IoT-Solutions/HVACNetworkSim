...
def main():
    ...
    chiller_loads = {"water_cooled": [], "air_cooled": []}
    chiller_cops = {"water_cooled": [], "air_cooled": []}
    tower_approaches = []
    boiler_loads = {"gas": [], "electric": []}
    boiler_efficiencies = {"gas": [], "electric": []}
    timestamp_hours = []
    outdoor_temps = []
    ...
    zone_temps_by_type = {"north": [], "south": [], "east": [], "west": []}

    ahu_airflows = {"AHU-1-North": [], "AHU-2-South": [], "AHU-3-East": [], "AHU-4-West": []}
    ...
    for hour in range(simulation_hours):
        ...
        updated_zone_temps = simulate_zone_temperatures(
            building.zones, current_outdoor_temp, hour, building.solar_ghi
        )
        ...
        gas_boiler.update_load(
            load=gas_boiler_load, entering_water_temp=160, hot_water_flow=500, ambient_temp=75
        )

        electric_boiler.update_load(
            load=electric_boiler_load, entering_water_temp=160, hot_water_flow=200, ambient_temp=75
        )
        ...
        total_cooling = (
            water_cooled_chiller.calculate_power_consumption()
            + air_cooled_chiller.calculate_power_consumption()
        )
        ...
        if hour % 6 == 0:
            print(
                f"Hour {hour}: Outdoor Temp: {current_outdoor_temp:.1f}°F, WB: {current_wet_bulb:.1f}°F"
            )
            print(
                f"  Water-Cooled Chiller: {water_cooled_chiller.current_load:.0f} tons, COP: {water_cooled_chiller.current_cop:.2f}"
            )
            print(
                f"  Air-Cooled Chiller: {air_cooled_chiller.current_load:.0f} tons, COP: {air_cooled_chiller.current_cop:.2f}"
            )
            print(
                f"  Cooling Tower Approach: {water_cooled_chiller.cooling_tower.calculate_approach():.1f}°F"
            )
            print(
                f"  Gas Boiler: {gas_boiler.current_load:.0f} MBH, Efficiency: {gas_boiler.current_efficiency*100:.1f}%"
            )
    ...
    gas_consumption = sum(
        gas_boiler.calculate_fuel_consumption()["therms_per_hour"]
        for _ in range(len(timestamp_hours))
    )
    ...
    print(
        f"Average Water-Cooled Chiller COP: {sum(chiller_cops['water_cooled'])/len(chiller_cops['water_cooled']):.2f}"
    )
    print(
        f"Average Air-Cooled Chiller COP: {sum(chiller_cops['air_cooled'])/len(chiller_cops['air_cooled']):.2f}"
    )
    print(
        f"Average Gas Boiler Efficiency: {sum(boiler_efficiencies['gas'])/len(boiler_efficiencies['gas'])*100:.1f}%"
    )
...
def plot_results(
    ...
    if valid_electric_eff:
        ax6.plot(
            valid_hours_electric,
            [e * 100 for e in valid_electric_eff],
            "purple",
