...
class Chiller(BACPypesApplicationMixin):
    ...
    def __init__(
        ...
        capacity: float,
        design_cop: float,
        design_entering_condenser_temp: float,
        design_leaving_chilled_water_temp: float,
        min_part_load_ratio: float,
        design_chilled_water_flow: float,
        design_condenser_water_flow: Optional[float] = None,
    ) -> None:
        """
    ...
    def update_load(
        self,
        load: float,
        entering_chilled_water_temp: float,
        chilled_water_flow: float,
        ambient_wet_bulb: Optional[float] = None,
        ambient_dry_bulb: Optional[float] = None,
    ) -> None:
        """
        ...
        if self.cooling_type == "water_cooled":
            ...
            estimated_cop = self._estimate_cop(
                limited_load, entering_chilled_water_temp, ambient_wet_bulb
            )
            ...
            self.cooling_tower.update_load(
                load=heat_rejection,
                entering_water_temp=entering_chilled_water_temp
                + self._calculate_condenser_delta_t(),
    ...
    def _calculate_performance(self, load: float) -> None:
        ...
        # delta_t calculated for future logging/diagnostics
        _ = self._calculate_delta_t(load)
    ...
    def _calculate_condenser_delta_t(self) -> float:
        ...
        delta_t = (heat_rejection * BTU_PER_TON_HR) / (
            WATER_HEAT_CONSTANT * self.condenser_water_flow
        )
    ...
    def _calculate_cop(self, load: float) -> float:
        ...
        if (
            hasattr(self, "old_setpoint")
            and self.old_setpoint != self.design_leaving_chilled_water_temp
        ):
    ...
    def _estimate_cop(
        self, load: float, entering_chilled_water_temp: float, ambient_wet_bulb: Optional[float]
    ) -> float:
    ...
    def get_process_variables(self) -> Dict[str, Any]:
        ...
        variables = {
            ...
            "chilled_water_delta_t": self.entering_chilled_water_temp
            - self.leaving_chilled_water_temp,
    ...
    def __str__(self) -> str:
        ...
        return (
            f"Chiller {self.name} ({self.cooling_type}): "
