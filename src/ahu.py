...
class AirHandlingUnit(BACPypesApplicationMixin):
    ...
    def _calculate_supply_air_temp(self, zone_temps):
        ...
        if cooling_count > heating_count:
            ...
            return max(self.min_supply_air_temp, self.supply_air_temp_setpoint - reset_amount)
            ...
            return min(self.max_supply_air_temp, self.supply_air_temp_setpoint + reset_amount)
    ...
    def _calculate_energy_usage(self) -> None:
        ...
        if self.cooling_valve_position > 0:
            ...
            if self.cooling_type == "chilled_water":
                ...
                efficiency_factor *= 0.8 + 0.2 * (
                    self.active_compressor_stages / self.compressor_stages
                )
        ...
        if self.heating_valve_position > 0:
            ...
            self.heating_energy = (
                mass_flow * AIR_SPECIFIC_HEAT * delta_t * self.heating_valve_position
            )
        else:
            self.heating_energy = 0
